...
exports.handler = async function (event) {
  ...
  try {
    ...
    const attachmentUrl = String(payload.attachmentUrl || payload.attachment_url || "").trim();
    const attachmentType = String(payload.attachmentType || payload.attachment_type || (imageData ? "image" : "")).trim();
    const attachmentName = String(payload.attachmentName || payload.attachment_name || imageName || "").trim();

    if (!sessionId) return sendJson(400, { ok: false, error: "Missing session_id" });
    if (!messageText && !imageData && !attachmentUrl) return sendJson(400, { ok: false, error: "Missing message text" });
    ...
    return sendJson(500, { ok: false, error: err && err.message ? err.message : "Could not send message" });
  }
};
